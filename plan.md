...
- **Phase 4 เขียนเสร็จ + E2E ผ่าน (2026-07-18)** — upload รูป (Vercel Blob) + ย่อ/ขยาย/ซูม/ลากจัดตำแหน่งรูป + hardening §8:
  ...
  - **ค้าง: ผูก Blob store จริง** — สร้าง Vercel project + Blob store แล้วใส่ `BLOB_READ_WRITE_TOKEN` ใน `.env` (หรือ `vercel env pull`) → ค่อยเทสต์อัพโหลดจริง end-to-end บน preview deployment (รวม assert ข้อความ Unauthorized/Not found/Invalid pathname ที่ local เห็นแค่ "No read-write token" เพราะ SDK เช็ค token ก่อนเรียก onBeforeGenerateToken — ยืนยันลำดับจาก source แล้วว่าเมื่อมี token เช็คของเรารันก่อนออก token เสมอ)
- **โหมด "ลองแก้ไข" บนหน้า public (2026-07-18, คำขอผู้ใช้)** — guest กดปุ่ม "ลองแก้ไข" ที่ toolbar `/r/[slug]` แล้วแก้ inline ได้ทุกอย่าง (ข้อความ/list/รูป ขนาด-ซูม-ลาก) บน**สำเนา client เท่านั้น**: ไม่มี autosave ไม่แตะ DB, มีป้าย "ไม่ถูกบันทึก", ปุ่ม "จบการแก้ไข"/รีเฟรชทิ้ง draft กลับเวอร์ชันจริง, แผงรูปซ่อนปุ่มอัพโหลด (ไม่มี resumeId/สิทธิ์) — implement ใน `ResumeDocumentView` (state `draft` + `<ResumeView editable>` ตัวเดิม), E2E ผ่าน 12/12 รวมเช็ค DB untouched
- **Editor เปลี่ยนจาก autosave เป็นปุ่มบันทึก (2026-07-18, คำขอผู้ใช้)** — แก้แล้วขึ้น "ยังไม่ได้บันทึก" (amber), กดปุ่มบันทึก/Ctrl+S เท่านั้นถึงเขียน DB, รีเฟรชโดยไม่บันทึก = ทิ้งการแก้ (มี beforeunload เตือน), **Publish ยัง flush draft ล่าสุดก่อนเผยแพร่เสมอ**; blob เก่าถูกลบตอนกดบันทึก (ใน updateDraft เหมือนเดิม); Gotcha: React StrictMode ใน dev รัน effect ซ้ำตอน mount — mark-dirty effect ต้องเทียบ identity ของ state (`lastSeen` ref) ห้ามใช้ flag firstRender ไม่งั้นเปิดหน้ามาก็ dirty เลย; E2E ผ่าน 11/11
- **Navbar ใช้ร่วมทุกหน้า (2026-07-18, คำขอผู้ใช้)** — `src/components/Navbar.tsx` (server component ใน root layout): แบรนด์ "Resume Builder" คลิกกลับ `/`, ปุ่มขวาตาม auth state (guest: เข้าสู่ระบบ/สมัครใช้งาน · login แล้ว: Dashboard/ออกจากระบบ), `print:hidden`, มือถือไม่ล้น; สิ่งที่ปรับตาม: landing ตัด nav ของตัวเอง, dashboard ตัดปุ่มออกจากระบบซ้ำ, login/signup เป็น `flex-1` (body เป็น flex-col), แถบควบคุมหน้า public เปลี่ยนจาก `fixed top-4` เป็น **sticky ใน flow ใต้ navbar** (ไม่ทับกัน, wrap บนมือถือ); **trade-off ที่ตัดสินใจแล้ว: auth() ใน layout ทำให้ทุกหน้า dynamic — landing เสีย ISR** แลกกับ navbar แสดงสถานะ login ถูกทุกหน้า; E2E ผ่าน 14/14 (รวม mobile 375px + เช็คไม่ทับ navbar; สร้าง user เทสต์ตรงใน DB เลี่ยง rate limit signup)
- Gotcha ปฏิบัติการ (Windows/dev): **ห้ามรัน `next build` ขณะ dev server รันอยู่** — แย่ง `.next` กันแล้ว dev พัง (routes-manifest หาย)/prisma generate EPERM; ผู้ใช้มักมี `npm run dev` รันอยู่ที่พอร์ต 3000 (ไม่ใช่ 3777 แล้ว) — เช็ค process ก่อน และอย่าเปิด dev สองตัวบนโฟลเดอร์เดียวกัน
