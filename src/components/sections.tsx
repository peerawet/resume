...
export function Profile() {
  ...
  return (
    <section>
      ...
      <p className="text-[14.5px] leading-[1.65]">{t.profile}</p>
    </section>
  );
}
  ...
  return (
    <section>
      ...
      <div className="flex flex-col gap-2.5">
        {t.techStack.map((cat) => (
          <div key={cat.label}>
            <div className="tracked mb-1.5 text-[13px] font-bold uppercase tracking-[0.06em] text-slate-500">
              {cat.label}
            </div>
            <div className="flex flex-wrap gap-1.5">
              {cat.items.map((item) => (
                <span
                  key={item}
                  className="rounded border border-slate-300 bg-white px-2 text-[13.5px] leading-[26px] text-slate-700"
                >
                  {item}
                </span>
              ))}
        ...
        {t.experience.map((item) => (
          <div key={`${item.role}-${item.period}`} className="relative">
            ...
            <div className="flex items-baseline justify-between gap-2">
              ...
              <span className="whitespace-nowrap text-[14px] font-semibold text-navy">
                {item.period}
              </span>
            </div>
            <div className="mb-1.5 text-[14px] text-slate-500">{item.company}</div>
            <ul className="list-disc space-y-1 pl-4 text-[14.5px] leading-[1.55] marker:text-slate-400">
    ...
    <section>
      ...
      <div className="flex flex-col gap-3.5">
        {t.education.map((item) => (
          <div key={item.title}>
            <div className="text-[15px] font-bold leading-snug">{item.title}</div>
            {item.detail && (
              <div className="mt-0.5 text-[14px]">{item.detail}</div>
            )}
            {item.highlight && (
        ...
        {t.projects.map((project) => (
          <div key={project.name}>
            <div className="flex flex-wrap items-center gap-2">
              <h3 className="text-[16.5px] font-bold">{project.name}</h3>
              {project.status && (
                <span
                  className={`tracked rounded border px-1.5 text-[12px] font-semibold uppercase leading-4 tracking-[0.06em] ${
              ...
              {project.links.map((link) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-0.5 text-[13px] font-semibold text-navy hover:underline"
                >
